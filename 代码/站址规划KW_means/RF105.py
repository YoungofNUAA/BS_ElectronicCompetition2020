...
#计算总的目标函数
def calcuObjective(preRSRP,low = -105):
    sum = 0
    count = 0
    for i in preRSRP:
        if i < low:
...
def myloc_clustering(centroidList,apTest):
    ...
    np.savetxt('./KMeans_BS_Result_Peking/RF_KMeans_BS_40_105_Rand2/Centroid.dat', centroidList)
    print('迭代结束')
...
while(abs(newObj-oldObj)>=1):
    ...
    if k>15:
        break
np.savetxt('./KMeans_BS_Result_Peking/RF_KMeans_BS_40_105_Rand2/RefreshRSRP.dat',refreshRSRP_apTest)
np.savetxt('./KMeans_BS_Result_Peking/RF_KMeans_BS_40_105_Rand2/LowPowerCount.dat',alllowPowerRatio)
